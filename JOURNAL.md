...
- [ ] （一次審査通過後）LIFF で参加者の ID を取得する：同じプロバイダーに LINE ログインチャネルを作り、LIFF アプリ（エンドポイント `/liff`、scope `profile`）を追加し、チャネルを公開する → `/liff` ページを実装して ID をログに出す → `users.json` に入れて実機デモを確認
- [ ] Dさんのスマホに、本番テストの提案が1通届いたか、ユーザーに確認してもらう
- [ ] Bさんに MukoBot へメッセージを送ってもらう → ID を取得して `users.json` に入れる（Aさん・Cさんは取得済み）→ 再デプロイ → 本番で実際に送って確認
- [ ] Phase 6（LINE で承認）はデモ対象外になった。やるなら Webhook の設定が必要
